...
- навигация по экранам `Home`, `Library`, `Collections`, `Add`;
...
- создавать локальные плейлисты и добавлять в них треки;
...
- `плейлисты`: хранятся локально рядом с библиотекой треков;
...
- альбомы и плейлисты тоже переживут закрытие приложения;
...
- плейлисты пока только ручные и без drag-and-drop сортировки;
...
На Windows токен теперь можно просто вставить обычным способом и нажать `Enter`. После этого бот будет отвечать в личном чате и показывать кнопку запуска Mini App.

Если хочешь принудительно оставить видимый режим, можно использовать:

```powershell
python bot.py run --show-token-input
```

И для первичной настройки:

```powershell
python bot.py setup --show-token-input
```

В этом режиме токен будет виден на экране во время вставки, но по-прежнему не будет храниться в файлах проекта.

Если нужен именно скрытый ввод токена:

```powershell
python bot.py run --hide-token-input
python bot.py setup --hide-token-input
```
